...
impl VisibilitySet {
    ...
    pub fn is_line_visible(&self, start_point: Vec2, end_point: Vec2) -> bool {
        ...
        fn is_cone_visible(set: &VisibilitySet, check_cone: Cone) -> bool {
            ...
            for blocking_cone in set.cones.iter() {
                let check_overlap_cone = match check_cone.overlapping_cone(*blocking_cone) {
                    None => continue,
                ...
                if check_overlap_cone.left_ray.distance < blocking_overlap_cone.left_ray.distance
    ...
    pub fn add_line(&mut self, start_point: Vec2, end_point: Vec2) -> Option<u32> {
        fn add_cone(set: &mut VisibilitySet, new_cone: Cone) -> bool {
            ...
            for blocking_cone in old_cones {
                ...
                let left_in_front =
                    new_overlap_cone.left_ray.distance < blocking_overlap_cone.left_ray.distance;
                let right_in_front =
                    new_overlap_cone.right_ray.distance < blocking_overlap_cone.right_ray.distance;
                ...
                let split_point = new_overlap_cone.left_ray.point
                    + (new_overlap_cone.right_ray.point - new_overlap_cone.left_ray.point)
                        * intersect_times.x;
            ...
            set.cones
                .sort_by(|a, b| a.left_ray.angle.total_cmp(&b.left_ray.angle));
        ...
        let visible = match Cone::create_from_line(start_point, end_point, self.next_id) {
...
impl Cone {
    fn create_from_line(start_point: Vec2, end_point: Vec2, line_id: u32) -> LineToCone {
    ...
    fn create_from_rays(left_ray: ConeRay, right_ray: ConeRay, line_id: u32) -> Cone {
        Cone {
            related_angle: Cone::derive_related_angle(left_ray.point, right_ray.point),
            left_ray,
            right_ray,
            line_id,
        }
    ...
    fn ray_at_angle(&self, angle: f32) -> ConeRay {
        ...
        ConeRay {
            angle,
            distance,
            point: distance * Vec2::from_angle(angle),
        }
    }

    fn overlaps_cone(&self, other: Cone) -> bool {
        other.left_ray.angle < self.right_ray.angle && self.left_ray.angle < other.right_ray.angle
    }
