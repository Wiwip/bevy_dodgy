...
pub fn solve_linear_program(
    ...
    match solve_linear_program_2d(constraints, radius, &OptimalValue::Point(preferred_value)) {
...
fn solve_linear_program_along_line(
    ...
    for constraint in constraints {
        ...
        let direction_determinant = determinant(line.direction, constraint.direction);
        let numerator = determinant(constraint.direction, line.point - constraint.point);
...
fn solve_linear_program_3d(
    ...
    for (index, constraint) in constraints[index_of_failed_line..].iter().enumerate() {
        if determinant(constraint.direction, constraint.point - best_value) <= penetration {
        ...
        penetration_constraints.extend_from_slice(&constraints[0..rigid_constraint_count]);
        ...
        for previous_constraint in &constraints[rigid_constraint_count..index] {
            ...
            if intersection_determinant.abs() <= RVO_EPSILON {
                ...
                intersection_point = (constraint.point + previous_constraint.point) * 0.5;
            } else {
                ...
                intersection_point = constraint.point + intersection_t * constraint.direction;
            };

            penetration_constraints.push(Line {
                direction: (previous_constraint.direction - constraint.direction).normalize(),
        ...
        if let LinearProgram2DResult::Feasible(result) = solve_linear_program_2d(
            ...
            best_value = result;
            penetration = determinant(constraint.direction, constraint.point - best_value);
        }
    }
    ...
    mod solve_linear_program_2d_tests {
        ...
        fn uses_projected_optimal_point() {
            ...
            assert_eq!(
                ...
                LinearProgram2DResult::Feasible(Vec2::new(one_over_root_2, one_over_root_2))
            );
        }
        ...
        fn constraints_are_infeasible() {
            ...
            assert_eq!(
                solve_linear_program_2d(&constraints, 1.0, &OptimalValue::Point(Vec2::ONE)),
                LinearProgram2DResult::Infeasible {
