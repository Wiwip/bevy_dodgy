...
impl Agent {
    ...
    pub fn compute_avoiding_velocity(
        ...
        let planes = neighbours
            .iter()
            .map(|neighbour| {
                self.get_plane_for_neighbour(neighbour, avoidance_options.time_horizon, time_step)
            })
            .collect::<Vec<Plane>>();
    ...
    fn get_plane_for_neighbour(
        ...
        if distance_squared > sum_radius_squared {
            ...
            let dot = cutoff_sphere_center_to_relative_velocity.dot(relative_neighbour_position);

            // TODO: Figure out why this works.
            if dot < 0.0
                && dot * dot
                    > sum_radius_squared * cutoff_sphere_center_to_relative_velocity_length_squared
            {
                ...
                vo_normal = cutoff_sphere_center_to_relative_velocity.normalize_or_zero();
                ...
                let tangent_ring_triangle_leg_squared = distance_squared - sum_radius_squared;
                ...
                vo_normal =
                    (relative_agent_velocity + t * relative_neighbour_position).normalize_or_zero();
                let distance_to_plane = Plane {
                    normal: vo_normal,
                    point: Vec3::ZERO,
                }
            ...
            vo_normal = (relative_agent_velocity - cutoff_sphere_center).normalize_or_zero();
    ...
    mod get_plane_for_neighbour_tests {
        ...
        fn collision_uses_time_step() {
            ...
            assert_plane_eq!(
                collision_plane,
                Plane {
                    point: (Vec3::new(1.0, 1.0, 0.0).normalize() * -8.0 + Vec3::new(4.0, 4.0, 0.0))
                        * 0.5,
