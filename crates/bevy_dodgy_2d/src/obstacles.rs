...
pub fn get_lines_for_agent_to_obstacle(
    ...
    match obstacle {
        Obstacle::Closed { vertices } => get_lines_for_agent_to_obstacle_const::<true>(
            agent,
            vertices,
            obstacle_margin,
            time_horizon,
        ),
        Obstacle::Open { vertices } => get_lines_for_agent_to_obstacle_const::<false>(
            agent,
            vertices,
            obstacle_margin,
            time_horizon,
...
pub fn get_lines_for_agent_to_obstacle_const<const CLOSED: bool>(
    ...
    if CLOSED {
        for left_index in 0..vertices.len() {
            let right_index = if left_index == vertices.len() - 1 {
                0
            } else {
                left_index + 1
            };
            let left_left_index = if left_index == 0 {
                vertices.len() - 1
            } else {
                left_index - 1
            };
            let right_right_index = if right_index == vertices.len() - 1 {
                0
            } else {
                right_index + 1
            };
...
pub fn get_line_for_agent_to_edge(
    ...
    let edge_t = agent_from_left_vertex.dot(edge_vector) / edge_vector.length_squared();
    let dist_to_edge_line_squared = (edge_t * edge_vector).distance_squared(agent_from_left_vertex);
    ...
    } else if 0.0 <= edge_t && edge_t <= 1.0 && dist_to_edge_line_squared <= squared_radius {
        return Some(Line {
            point: Vec2::ZERO,
            direction: -edge_unit_vector,
        });
    ...
    if edge_t < 0.0 && dist_to_edge_line_squared <= squared_radius {
        ...
        if left_vertex.convex {
            ...
            // present).
            let left_tangent_triangle_leg = (dist_left_squared - squared_radius).sqrt();
            left_shadow_direction = (relative_left_vertex * left_tangent_triangle_leg
                + relative_left_vertex.perp() * edge_margin)
                / dist_left_squared;
        } else {
        ...
        if right_vertex.convex {
            let right_tangent_triangle_leg = (dist_right_squared - squared_radius).sqrt();
            right_shadow_direction = (relative_right_vertex * right_tangent_triangle_leg
                - relative_right_vertex.perp() * edge_margin)
                / dist_right_squared;
        } else {
    ...
    let left_edge_direction = left_left_vertex.and_then(|v| Some(v - left_vertex.point));
    if left_vertex.convex
    ...
    let right_edge_direction = right_right_vertex.and_then(|v| Some(v - right_vertex.point));
    if right_vertex.convex
        ...
        || is_degenerate_edge && t_left_shadow < 0.0 && t_right_shadow < 0.0
    {
        ...
        return Some(Line {
            ...
            point: left_cutoff + edge_margin / time_horizon * velocity_from_left_cutoff,
        });
    }
    ...
    if t_cutoff_edge > 1.0 && t_right_shadow < 0.0 {
        ...
        return Some(Line {
            ...
            point: right_cutoff + edge_margin / time_horizon * velocity_from_right_cutoff,
        });
    }
    ...
    let left_shadow_distance_squared = if t_left_shadow < 0.0 {
        ...
        (velocity - (left_cutoff + t_left_shadow * left_shadow_direction)).length_squared()
    };
    let right_shadow_distance_squared = if t_right_shadow < 0.0 {
        f32::INFINITY
    } else {
        (velocity - (right_cutoff + t_right_shadow * right_shadow_direction)).length_squared()
    };
        ...
        && cutoff_edge_distance_squared <= right_shadow_distance_squared
    {
        ...
        if is_left_shadow_covered {
            ...
            Some(Line {
                ...
                point: left_cutoff + edge_margin / time_horizon * left_shadow_direction.perp(),
            })
        }
    } else {
        if is_right_shadow_covered {
            ...
            Some(Line {
                ...
                point: right_cutoff - edge_margin / time_horizon * right_shadow_direction.perp(),
            })
        }
    }
}
...
mod tests {
    ...
    fn covered_edge_is_skipped() {
        ...
        assert!(
            line.is_some(),
            "Line should have been Some since the existing line did not fully cover the edge."
        );
        ...
        assert!(
            line.is_none(),
            "Line should have been None due to an existing line covering the edge. Line was {:?}",
            line
        );
    }
    ...
    fn agent_collides_with_right_vertex_handled_by_next_edge() {
        ...
        assert!(
            line.is_none(),
            "The right vertex should be handled by the next edge. The generated line was {:?}",
            line.unwrap()
        );
        ...
        assert!(
            line.is_some(),
            "The right vertex should be handled by this edge."
        );
    }
    ...
    fn agent_velocity_projects_to_covered_shadows_creates_no_lines() {
        ...
        assert!(
            line.is_some(),
            "Left shadow was covered for the right edge, so this edge should create a line."
        );
        ...
        assert!(
            line.is_some(),
            "Right shadow was covered for the left edge, so this edge should create a line."
        );
    }
    ...
    fn shadow_of_endpoint_covers_edge() {
        ...
        assert_line_eq!(
            get_line_for_agent_to_edge(
                &agent,
                EdgeVertex {
                    point: Vec2::new(-1.0, 4.0),
                    convex: true
                },
                EdgeVertex {
                    point: Vec2::new(0.0, 2.0f32.sqrt()),
                    convex: true
                },
                None,
                None,
