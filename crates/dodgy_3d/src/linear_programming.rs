...
pub fn solve_linear_program(constraints: &[Plane], radius: f32, preferred_value: Vec3) -> Vec3 {
    match solve_linear_program_3d(constraints, radius, &OptimalValue::Point(preferred_value)) {
        LinearProgram3DResult::Feasible(optimal_value) => optimal_value,
        LinearProgram3DResult::Infeasible {
            index_of_failed_line,
            partial_value,
        } => solve_linear_program_4d(constraints, radius, index_of_failed_line, partial_value),
    }
}
...
fn solve_linear_program_along_line(
    ...
    let discriminant =
        line_dot_product * line_dot_product + radius * radius - line.point.dot(line.point);
...
fn solve_linear_program_along_plane(
    ...
    let squared_radius_in_valid_plane = radius_squared - plane_distance_squared_from_origin;
    ...
    let mut best_value = match optimal_value {
        &OptimalValue::Direction(direction) => {
            ...
            let squared_length_of_projection =
                projected_optimal_direction_in_plane.dot(projected_optimal_direction_in_plane);

            if squared_length_of_projection <= RVO_EPSILON {
                valid_plane_center
            } else {
                valid_plane_center
                    + (squared_radius_in_valid_plane / squared_length_of_projection).sqrt()
        ...
        &OptimalValue::Point(point) => {
            ...
            if projected_point.dot(projected_point) > radius_squared {
                ...
                valid_plane_center
                    + (squared_radius_in_valid_plane / squared_distance_from_center).sqrt()
                        * relative_point
            } else {
                projected_point
            }
    ...
    for (index, constraint) in constraints.iter().enumerate() {
        ...
        let Ok(new_value) =
            solve_linear_program_along_line(&line, radius, &constraints[0..index], optimal_value)
        else {
            return Err(());
        };
...
fn solve_linear_program_4d(
    ...
    for (index, constraint) in constraints[index_of_failed_plane..].iter().enumerate() {
...
mod tests {
    ...
    mod solve_linear_program_along_plane_tests {
        ...
        use crate::linear_programming::{solve_linear_program_along_plane, OptimalValue};
    ...
    mod solve_linear_program_3d_tests {
        ...
        fn uses_optimal_direction() {
            ...
            assert_vec3_near!(
                unwrap_feasible(solve_linear_program_3d(
                    Default::default(),
                    3.0,
                    &OptimalValue::Direction(Vec3::new(0.0, one_over_root_2, one_over_root_2)),
                )),
        ...
        fn constraints_are_infeasible() {
            ...
            let (index_of_failed_line, partial_value) = unwrap_infeasible(solve_linear_program_3d(
                &constraints,
                1.0,
                &OptimalValue::Point(Vec3::ZERO),
            ));
