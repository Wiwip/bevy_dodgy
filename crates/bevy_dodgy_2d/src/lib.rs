...
impl Agent {
    ...
    pub fn compute_avoiding_velocity(
        ...
        let lines = obstacles
            ...
            .chain(neighbours.iter().map(|neighbour| {
                self.get_line_for_neighbour(neighbour, avoidance_options.time_horizon, time_step)
            }))
            .collect::<Vec<Line>>();
    ...
    fn get_line_for_neighbour(&self, neighbour: &Agent, time_horizon: f32, time_step: f32) -> Line {
