...
pub(crate) fn rvo_avoidance(
    ...
    for (entity, agent, tf, goal, options) in agents.iter() {
        let intersections = spatial.shape_intersections(
            &Collider::circle(agent.radius + options.time_horizon * agent.max_speed), // Shape
...
pub(crate) fn create_collider(
    ...
    query: Query<(Entity, &Agent), Added<Agent>>,
) {
    for (e, agent) in query.iter() {
